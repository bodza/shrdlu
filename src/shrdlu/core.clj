...
(DEFPROP TC-FINDSPACE
    (THCONSE (SURF SIZE OBJ SPACE)
        ...
        (THOR (AND (NOT (MEMQ $?SURF (QUOTE (:BOX :TABLE))))
                (NOT (GET (QUOTE #NOCLEAR) 'THASSERTION))
                (THSETQ $_SPACE (FINDSPACE 'CENTER
                        $?SURF
                        $?SIZE
                        $?OBJ)))
        (AND (OR (EQ $?SURF (QUOTE :BOX))
                (AND (NOT (EQ $?SURF (QUOTE :TABLE)))
                    (GET (QUOTE #NOCLEAR)
                    'THASSERTION)))
                (THSETQ $_SPACE (FINDSPACE 'PACK
                        $?SURF
                        $?SIZE
                        $?OBJ)))
        (THSETQ $_SPACE (FINDSPACE 'RANDOM
                        $?SURF
                        $?SIZE
                        $?OBJ))))
    THEOREM)
            ...
            ((THGOAL (#GRASPING $?X))
                ...
                (OR NOMEM
                    (THPUTPROP $?X
                        (CONS (LIST THTIME
                                $?Z
                                (CADAR (OR (THVAL (QUOTE (THGOAL (#SUPPORT $?Y $?X)))
...
(DEFPROP TC-PACK
    ...
    GO  (THCOND ((NULL $?BLOCKS)
            (THCOND ((NULL $?PYR) (THSUCCEED THEOREM))
                ((THVSETQ $_Y (FINDSPACE 'PACK
                            $?SURF
                            (SIZE (CAR $?PYR))
                            (CAR $?PYR)))
...
(DEFPROP TC-STACKUP
    (THCONSE (X Y BLOCKS PYR (WHY (EV)) EV)
        ...
        (MEMORY)
    GO  (THCOND ((CDR $?BLOCKS) (THGOAL (#PUTON $E (CADR $?BLOCKS) $E (CAR $?BLOCKS))
                        (THUSE TC-PUTON))
...
(DEFPROP TC-STARTEND4
    (THCONSE (X NEWEV Z EV TIME)
        ...
        (TIMECHK $?NEWEV $?TIME)
        (PUTPROP $?NEWEV $?Z 'WHY)
        (PUTPROP $?NEWEV (QUOTE #START) 'TYPE))
    THEOREM)
...
(DEFPROP TCTE-PICKUP
    (THCONSE (X EV EVENT TIME)
        ...
        (MEMQ (GET $?EVENT 'TYPE) (QUOTE (#PUTON #GET-RID-OF)))
        (TIMECHK $?EVENT $?TIME)
...
(DEFPROP TCTE-PUT
    (THCONSE (X Y EV EVENT TIME Z)
        (#PUT $?EV $?X $?Y $?TIME)
        (THAMONG $?EVENT EVENTLIST)
        (MEMQ (GET $?EVENT 'TYPE) (QUOTE (#PICKUP #PUTON)))
        (TIMECHK $?EVENT $?TIME)
        (THOR (THGOAL (#PUTON $?EVENT $?X ?))
        (THGOAL (#PICKUP $?EVENT $?X)))
        (OR (THVSETQ $_Z (SUB1 (ASSQ (GET $?EVENT 'END)
                        (GET $?X 'HISTORY))))
            (ERT TCTE-PUT WRONG))
        (THAMONG $?Y (LIST (CADR $?Z)))
        (THSETQ $_EV (MAKESYM 'E))
        (AND (PUTPROP $?EV
            (PUTPROP $?EV (CAR $?Z) 'START)
            'END)
            (PUTPROP $?EV $?EVENT 'WHY)
            (PUTPROP $?EV (QUOTE #PUT) 'TYPE)
            (SETQ EVENTLIST (CONS $?EV EVENTLIST))
            (THASSERT (#PUT $?EV $?X $?Y))))
    THEOREM)
...
(DEFPROP TCT-GRASP
    ...
    UP  (THCOND ((OR (NULL $?Z) (STARTIME $?Z $?TIME)) (THFAIL))
...
(QUOTE (COMMANDS ARE: #CLEARTOP #GET-RID-OF #GRASP #PACK #PICKUP #PUTIN #PUTON #RAISEHAND #STACKUP #UNGRASP))
...
(DEFUN CLEAR (LOC SIZE OBJ)
    (PROG (W X1 X2)
        ...
        (AND (MEMQ NIL (MAPCAR '(LAMBDA (X Y) (AND (GREATERP X -1) (GREATERP 1201 (PLUS X Y)) T)) LOC SIZE))
            (RETURN NIL))
        (SETQ W ATABLE)
...
(DEFUN GOAL FEXPR (X)
    (SETQ PLAN NIL)
    (THVAL (LIST 'THGOAL (CAR X) (QUOTE (THTBF THTRUE)))
        (QUOTE ((EV COMMAND))))
    (EVLIS (REVERSE PLAN)))
        ...
        (COND
            ((OR
                ...
                (NULL (ERRSET
                    ...
                    ATABLE))))
                (RETURN NIL)))
    GO   (COND ((= (SETQ GROW (MIN (CAAR XL) (CAAR XH) (CAAR YL) (CAAR YH))) 2000)
...
(DEFUN LOCGREATER (X Y FN)
    ((LAMBDA (XX YY)
        (NOT (LESSP (FN (CADR XX)) (PLUS (FN (CADR YY)) (FN (CADDR YY))))))
        (LOCG2 (QUOTE $?YY) X)
        (LOCG2 (QUOTE $?ZZ) Y)))

(DEFUN LOCG2 (X Y) (COND ((EQ $?LOC (QUOTE #LOC)) (ATAB Y)) ((CONS NIL (CONS (EVAL X) (CDDR (ATAB Y)))))))

(DEFUN MEMOREND FEXPR (A)
    (OR NOMEM (AND (PUTPROP $?EV THTIME 'END)
            (APPLY 'THASSERT
            (LIST (THVARSUBST (CAR A) NIL)))
            (PUTPROP $?EV (CAAR A) 'TYPE))))

(DEFUN MEMORY NIL
    (OR NOMEM (THAND (THVSETQ $_EV (MAKESYM 'E))
...
(DEFUN PACKO (OBJ TYPE)
    (PROG (XX)
        (MAPC (FUNCTION
            (LAMBDA (X)
                (AND (THVAL (QUOTE (THGOAL (#IS $?X $E TYPE)))
                    (LIST (LIST 'X X)))
...
(DEFUN PACKON (SURF LIST)
    ...
    GO  (COND ((NULL LIST) (RETURN NIL))
            ((OR
                ...
                (GREATERP (PLUS (CADDR X) (CADDR (CADR SURF)) (CADDR (CADDR SURF))) 501))
            (SETQ LIST (CDR LIST))
            (GO GO))
            ((RETURN (CAR X))))))
...
(DEFUN TFIND (X Y)
    ...
    UP  (COND ((NOT (GREATERP (CAAR Z) (OR (END? Y) 77777)))
            (RETURN Z))
            ((SETQ Z (CDR Z)) (GO UP)))))
...
(MAPC (QUOTE (LAMBDA (X Y) (PUTPROP X (LIST Y) 'COLOR)))
...
(DECLARE (*FEXPR THAPPLY THGENAME THSTATE THANTE THERASING THCONSE THDUMP THRESTRICT THBKPT THUNIQUE THVSETQ THMESSAGE THDO THGOAL THERASE THAND THNV THSUCCEED THAMONG THCOND THSETQ THASSERT THASVAL THERT THGO THFAIL THOR THFIND THFINALIZE THRETURN THPROG THFLUSH THNOT THV))
...
(DEFUN THREAD NIL
    ...
    ;; EXPANDS _ TO (THNV (READ)) EXPANDS A TO ASSERT
    ;; EXPANDS G TO GOAL EXPANDS T TO THTBF THTRUE
    ;; EXPANDS ? TO (THV (READ)) EXPANDS E TO (THEV (READ))
    ;; EXPANDS R TO THRESTRICT
    (PROG (CHAR)
        ;; TREATS & - - & AS A COMMENT
        (RETURN (COND
            ((EQ (SETQ CHAR (READCH)) (QUOTE ?)) (LIST 'THV (READ)))
            ((EQ CHAR 'E) (LIST 'THEV (READ)))
            ((EQ CHAR (QUOTE _)) (LIST 'THNV (READ)))
            ((EQ CHAR (QUOTE &))
                (PROG NIL
...
(DEFUN THASS1 (THA P)
    (PROG (THX THY1 THY TYPE PSEUDO)
        ...
        (OR PSEUDO
            (THPUSH THTREE
                (LIST (COND (P 'THASSERT) ('THERASE)) THX THY)))
...
(DEFUN THGAL (X Y)
    ...
    (SETQ THXX X)
    (SASSQ (CADR X) Y (FUNCTION (LAMBDA NIL (PRINT THXX) (THERT THUNBOUND THGAL)))))
...
(DEFUN THGOAL FEXPR (THA)                                   ;; THA = (PATTERN RECOMMENDATION)
    (PROG (THY THY1 THZ THZ1 THA1 THA2)                     ;; PATTERN IS EITHER EXPLICIT, THE VALUE OF A
        ...
        (SETQ THA1 (MAPCAN (FUNCTION THTRY) THA1))          ;; THMS AND ASSERTIONS SATISFYING RECS APPENDED TO RECS
...
(DEFUN THIP (THI)
    ...
    (PROG (THT1 THT3 THSV THT2 THI1)
        ...
        (COND ((AND (ATOM THI) (NOT (EQ THI (QUOTE ?))) (NOT (NUMBERP THI)))
            ...
            (COND
                ...
                ;; BINDING LIST, WE MAKE UP A PSEUDO BINDING LIST
                    (COND ((EQ (CADR THX) (QUOTE ?))
                            (PROG2 0
                                (CONS (QUOTE ?) (CONS 'THUNASSIGNED (APPEND (CDDR THX) NIL)))
                                (SETQ THX (QUOTE (THNV ?)))))
            ...
            (COND ((THVAR THY) (THGAL THY THALIST))
                ((EQ (CAR THY) 'THRESTRICT)
                    (COND ((EQ (CADR THY) (QUOTE ?))
                        (PROG2 0
                            (CONS (QUOTE ?) (CONS 'THUNASSIGNED (APPEND (CDDR THY) NIL)))
                            (SETQ THY (QUOTE (THNV ?)))))
                        (T ((LAMBDA (U)
...
(DEFUN THMATCHLIST (THTB THWH)
    ...
    THP3 (COND ((OR (NULL (ATOM THB2))
        ...
        (COND
            ((NOT (SETQ THA2 (GET 'THVRB THWH))) (SETQ THA2 (QUOTE (0 0))))
...
(DEFUN THSTATE FEXPR (THINDICATORS)
    ;; PRINTS THAT PART OF THE STATE OF THE MICRO-PLANNER WORLD SPECIFIED BY THE INDICATORS
    ;; IN REREADABLE FORM. NOTE THAT IT IS BLIND TO ASSERTIONS THAT BEGIN WITH
    ;; EITHER NUMBERS, LIST STRUCTURE, NOHASHED ATOMS OR NON-INTERNED ATOMS.
    (PROG (THP)
        (PRINT (QUOTE (THDATA)))
        (MAPC (FUNCTION (LAMBDA (BUCKET)
            (MAPC (FUNCTION (LAMBDA (THATOM)
                (MAPC (FUNCTION (LAMBDA (THWH)
                    (AND
...
(DEFUN THTRY1 NIL                                           ;; TRIES NEXT RECOMMENDATION ON TREE FOR THGOAL
    ...
    THTBF1 (COND ((NOT (AND (SETQ THW                       ;; TRY NEXT REC
                    (GET THEOREM 'THEOREM))
                    (EQ (CAR THW) 'THCONSE)))
            (PRINT THEOREM)
...
(DEFUN THUNIQUE FEXPR (THA)
    (SETQ THA (CONS 'THUNIQUE (MAPCAR (FUNCTION EVAL) THA)))
    (PROG (X)
        (SETQ X THALIST)
...
(DEFUN THV FEXPR (X)            ;; (THV X) IS THE VALUE OF THE PLANNER VARIABLE $?X
    (THV1 (CAR X)))

(DECLARE (SPECIAL THLEVEL THSTEP THSTEPF THSTEPT THSTEPD THMESSAGE ^A THV THINF THE THTREE THOLIST THEXP THALIST THVALUE THBRANCH THABRANCH))
...
(DECLARE (UNSPECIAL THSTEP THSTEPF THSTEPT THSTEPD THLEVEL THMESSAGE ^A THV THINF THE THTREE THOLIST THEXP THALIST THVALUE THBRANCH THABRANCH))
...
(DEFUN THINIT FEXPR (L)
    ...
    (SETQ ERRLIST
        (QUOTE ((PRINT 'MICRO-PLANNER)
            (PRINC THVERSION)
            (COND ((ERRSET (APPLY 'UREAD (APPEND '(/.PLNR/. /(INIT/)) (CRUNIT))) NIL)
...
(DEFUN THSEL (THF)
    (PROG (THX) (RETURN (AND (SETQ THX (ASSQ THL THTRACE))
        (SETQ THX (THF THX))
        (THVAL THX THALIST)))))
...
(DEFUN TIMER (T0 T1) (QUOTIENT (- T1 T0) 1000000))
...
(DEFUN TIME-SINCE (X) (QUOTIENT (- (RUNTIME) X) 1000000))
...
(DEFUN CLEANUP FEXPR (SYMBOL-LIST)
    ...
    (MAPC '(LAMBDA (SYMBOL)
            (CLEANX SYMBOL
                (GET SYMBOL 'OLD)
                (PUTPROP SYMBOL (GET SYMBOL 'NEW) 'OLD))
            (PUTPROP SYMBOL (GET SYMBOL 'MAKESYM) 'NEW))
        SYMBOL-LIST))
...
(DEFUN ERTEX (MESSAGE CAUSE-ABORTION IGNORE-NOSTOP-SWITCH?)
    ...
    PRINT
        ...
        (COND (ZOG-USER (PRINT 'LISTENING--->)) (T (PRINT '>>>)))
    LISTEN
...
(DEFPROP PR1
    (LAMBDA (A)
        (COND ((ATOM (H A)) (LIST (WORD (NB A)) (FE A)))
            ((PR2 (SM A))
            (LIST (FROM (NB A) (N A)) (FE A) (SM A) (COND ((ATOM (H A)) '\ ) ((MAPLIST (FUNCTION PR1) (REVERSE (H A)))))))))
    EXPR)
...
(DEFUN ETAOIN NIL
    ...
    CHAR (COND ((EQUAL (TYIPEEK) 24) (READCH) (ERT) (GO THRU)) ;; "CNTRL-X" BREAK LEFT OVER FROM CMU
        ...
        (SETQ CHAR (COND
            ((GREATERP 123 (SETQ CHAR (TYI)) 96) (- CHAR 32))
            ...
            ;; PROPERTY OF THE CURRENT READTABLE, BUT THIS WAS QUICKER TO PATCH.
            CHAR (ASCII CHAR))
    ...
    TRY (COND
            ((OR
                (SETQ FEATURES (GET (SETQ ROOT (READLIST (REVERSE RD))) 'FEATURES))
    ...
    WRD (SETQ SENT
            (COND (POSS
                (COND
                    ((OR (MEMQ 'NOUN
                        (SETQ FEATURES (GET WRD 'FEATURES)))
                        (MEMQ 'PROPN FEATURES))
    ...
    PUNC (COND
            (PUNCT (COND
                ((AND (EQ PUNCT '?) (NULL SENT)) (HELP) (GO THRU))
                ((MEMQ PUNCT FINAL)
...
(DEFUN BCWL FEXPR (A) ;; DEFINES COMBINATIONS OF WORDS
    (MAPC '(LAMBDA (X)
        (MAPC '(LAMBDA (Y)
            (BUILDWORD
                (INTERN (MAKNAM (NCONC (EXPLODE (CAR X)) (CONS '- (EXPLODE (CAR Y))))))
...
(DEFUN BUILDWORDLIST FEXPR (A) ;; DEFINES WORDS ;; ROOT IS OPTIONAL
    (MAPC '(LAMBDA (X)
...
(DEFUN ETNEW NIL
    (AND (EQ (CAR WORD) '")
        ...
        (BUILDWORD WRD '(NOUN NS) '((NOUN (NEWWORD))) NIL)))
...
(DEFUN LABELTRACE FEXPR (A)
    (MAPC '(LAMBDA (X)
        (PROG (BODY)
            (PRINT X)
            (COND ((GET X 'LABELTRACED) (PRINC 'ALLREADY-) (GO TRACED))
...
(DEFUN UNLABELTRACE FEXPR (A)
    (MAPC '(LAMBDA (X)
        (PROG (BODY)
            (PRINT X)
            (COND
                ((NOT (GET X 'LABELTRACED)) (PRINC 'ISN/'T\ ALLREADY-) (GO TRACED))
                ((GET X 'INTERPRET) (SETQ BODY (CDR (GET X 'INTERPRET))))
                ((GET X 'EXPR) (SETQ BODY (CDDR (CADDR (GET X 'EXPR)))))
                (T (PRINC 'CAN/'T\ BE-) (GO TRACED)))
...
(DEFUN SHOWSCENE (X)
    (PROG (PLANNERSEE)
        ...
        (MAPC '(LAMBDA (OBJ)
            ...
            (AND (SETQ OBJ
                    (THVAL '(THFIND ALL $?X (X) (THGOAL (#SUPPORT $?OBJ $?X)))
                        (LIST (LIST 'OBJ OBJ))))
                    (TAB 13)
                    (PRINC 'SUPPORTS\ )
                    (PRINC OBJ)))
...
(DEFS STOP
    ...
    (LAMBDA (X) (SETQ DPSTOP (ONOFF X
                    '(STOP AFTER DISPLAYING EACH NODE AND SEMANTIC STRUCTURE?)))
        (SETQ PLANNERSEE
            (AND PLANNERSEE
                (COND ((ONOFF X
...
(DEFS MARKER
    ...
    (LAMBDA (X)
        ...
        (PUTPROP (SETQ X (CHARG X
                ...
                (LIST (SETQ Y
                    (REQUEST 'PARENT:
                        '(NODE TO WHICH IT IS ATTACHED IN THE TREE))))
                'SYS)
        (PUTPROP Y
                (CONS X (GET Y 'SYSTEM))
                'SYSTEM)))
...
(DEFUN DEFINE FEXPR (A)
    (PROG (FE TYPE MARK REST TR)
        (SETQ A  (COND  (A (CAR A))
                    (T  (REQUEST 'WORD: '(ENGLISH WORD TO BE DEFINED)))))
        (SETQ TYPE
            (QUERY '(NOUN OR VERB?)
                '(NOUN VERB)
    ...
    MAR (OR (SETQ MARK (REQUEST 'MARKERS:
        ...
        (PUTPROP A
            (LIST (LIST 'VB (LIST 'RELATION (LIST 'MARKERS: MARK 'RESTRICTIONS: REST
                'PROCEDURE:
                (LIS2FY (REQUEST 'PROCEDURE:
...
(DEFUN HELP NIL
    (COND ((EQ 'S
        (QUERY '(TYPE L FOR LONG FORM (85 LINES) S FOR SHORT (16 LINES))
            '(S L)
            NIL))
...
;;              (EVAL V)
;;              (GO MORE)))))
;;      (T (SETQ PH-TURN-ON NIL GP-LINES NIL GP-SURFACE NIL GP-HANDIT NIL GP-NEWOBLOCAT NIL PH-BLOCKS NIL)
;;          (GP-INITIAL)))
;;  (TERPRI)
...
(DEFUN CALLSM FEXPR (SEMANTIC-EXPRESSION)
    (PROG (RESULT MPLNR-TIME SM-TTIME GC SMNFN)
        ...
        (SETQ MPLNR-TIME 0)
        (SETQ GC (STATUS GCTIME) SM-TTIME (RUNTIME) RESULT (EVAL (CAR SEMANTIC-EXPRESSION)))
        (SETQ SM-TIME (PLUS SM-TIME (DIFFERENCE (TIMER SM-TTIME (RUNTIME)) MPLNR-TIME)))
        (OR (= GC (STATUS GCTIME))
            (SETQ SM-TIME (DIFFERENCE SM-TIME (TIMER GC (STATUS GCTIME))) P-GC (STATUS GCTIME)))
...
(DEFUN FQ FEXPR (A)
    (MAPCAR (FUNCTION (LAMBDA (X) (OR (MEMQ X FE) (SETQ FE (CONS X FE))))) A)
    (SETR 'FEATURES FE C))
...
(DEFUN INTERPRET (UNIT)
    ...
    (PROG (FE H ME NB C SM CUT NN T1 T2 T3)
        ...
        (COND ((EQ (APPLY 'PROG (GET UNIT 'INTERPRET))
            'RETURN)
        ...
        (RETURN (COND
            ...
            ((SETQ ANS (THVAL-MULT (PLNR-FINDIFY 'ALL VAR (LIST VAR) (AMBPUT CODE))))
                (ANSBUILD
                    ...
                    (COND ((NULL (CADR ANS)) '((SAY I CAN/'TDISCUSSA NON-EXISTENT EVENT)))
...
(DEFUN THVAL-MULT (CODE)
    ...
    (PROG (ANS)
        ...
        ;; GIVE A VALUE OF 400 IF HE COULDN'T HAVE ANSWERED IT AT ALL.
        ...
        ;; 200 IF HE COULD ANSWER IT BUT NOT WITH RECENT INFO.
...
(DEFPROP IASS
    (LAMBDA (X)
        (PROG (XX)
            (OR (SETQ XX (CADR (SASSQ X (CADR (CADDDR ANS)) (FUNCTION SASS))))
                (RETURN T))
            (SAY /
BY)         (PRINC (COND ((EQ X 'IT) (QUOTE "IT")) ((MEMQ 'THEY (FROM SENT NIL)) (QUOTE "THEY")) ((QUOTE "THEM"))))
            (SAY , I ASSUME YOU)
            (PRINC 'MEAN)
...
(DEFUN COMMA NIL
    (COND ((SECONDWORD? '") (FLUSHME) T) ((CONJ)) ((ISQ RE INIT) (FLUSHME) T)))  ;; %% '"sic!
...
(DEFUN LOADSHRDLU NIL
    (ALLOC '(LIST 60000 FIXNUM 15000 SYMBOL 15000 ARRAY 500))
    (SETQ PURE NIL)
...
(DEFUN LOADSHRDLU NIL
    (ALLOC '(LIST 320000 FIXNUM 15000 SYMBOL 15000 ARRAY 500))
    (SETQ PURE NIL)
    (MAPC 'LOADER '(PLNR THTRAC))
    (THINIT)
    (SETQ THINF NIL THTREE NIL THLEVEL NIL)
...
(SETQ MAKEINTERN T              ;; SWITCH FOR INTERNING THE ATOMS CREATED FOR THE NODE STRUCTURE
      SH-STANDARD-PRINTOUT NIL  ;; SWITCH FOR EVALUATING DISPLAY FUNCTIONS IN THE FUNCTION SHSTPO (THE SHOW FILE)
      SH-AFTERANSWER-PAUSE T)   ;; SWITCH FOR CAUSING A BREAK AFTER EACH SENTENCE IS PROCESSED
...
CATCH-LOOP
        ...
        (AND IGNORE (GO UP))
        (COND
            ((AND (COND (TOPLEVEL-ERRSET? (ERRSET (SETQ PT (SETQ C (PARSEVAL PARSEARGS))))) (T (SETQ PT (SETQ C (PARSEVAL PARSEARGS))))) C)
                (OR ANNOYANCE (PRINT *2))
                (SETQ FE (FE C))
                (SETQ NB SENT)
                (SETQ H (H C))
                ...
                (PRINC P-TIME))
                ((PRINT *3) (APPLY 'SAY (OR GLOBAL-MESSAGE '(I DON/'T UNDERSTAND/.)))))
        (AND SH-STANDARD-PRINTOUT (SHSTPO))
        (AND SH-AFTERANSWER-PAUSE (ERT))
        (GO LOOP))
        ABORT-PARSER)
...
(DEFUN ETAOIN NIL ;; HAS A PATCH ADDED TO PERMIT ONLINE DEFINITION OF AN UNKNOWN WORD'S SYNTACTIC FEATURES
    (PROG (WORD NEWWORD CHAR ALTN ALREADY-BLGING-NEWWRD WRD LAST FEATURES NEXT Y WORD1 X RD POSS)
    THRU (SETQ SENT (SETQ WORD (SETQ PUNCT (SETQ POSS NIL))))
        (PRINT 'READY)
        (TERPRI)
    CHAR (COND ((EQUAL (TYIPEEK) 24) (READCH) (ERT) (GO THRU)) ;; "CNTRL-X" BREAK LEFT OVER FROM CMU
...
(DEFUN BUILD FEXPR (FOO)
    ;;  THIS IS A SEMANTIC FUNCTION WHICH PACKAGES SEMANTIC NODES
    T)
