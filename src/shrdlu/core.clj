        ...
        (let [a (if (term? *thtt*)
                    (let [x (getprop *thtt* :theorem)]
                        ...
                        (loop-when-recur thpl thpl (cddr thpl)
                            (thputprop *thtt* (keyword (car thpl)) (cadr thpl)))
                        (caddr x))
            ...
            :else
                (or (getprop *wrd* :features)     ;; IF A WORD HAS FEATURES, IT'S PROPERTIES ARE ALL SET UP IN THE DICTIONARY
                    (let [x (getprop *wrd* :irregular)]
                        (cond x
                                (buildword *wrd* (setmod (features x) (cdr x)) (semantics x) (car x))
                            (= (last *word*) '=)
    ...
    TRY (SETQ root (symbol* (reverse *rd*)))
        (let [features (getprop root :features)]
            (cond (or features (let [x (getprop root :irregular)] (and x (SETQ features (setmod (getprop (SETQ root (car x)) :features) (cdr x))))))
                    (buildword *wrd* (setmod features (getr *word* :mod)) (getprop root :semantics) root)
...
(putprop! 'DO :semantics [['VB [['TRANS '(relation [:restrictions [['(!ANIMATE)] ['(!EVENT)]] :markers ['!EVENT]])]]]])
...
(defn- plnr-recommendify [a]
    ...
    (let-when [e (getprop (car a) :thmlist)] e
        ...
        (eval (if (number? (caar e)) (cadr (assq (count a) e)) e))))
...
(defn- compare-build [node degree]
    ...
    (let [{:keys [restrictions dimension direction]} (cdr (findmeasure node))]
        (putprop! 'COMPARE-PSEUDO-VERB :semantics
            (list 'relation
                    [:restrictions (list (list restrictions) (list restrictions))
...
(defn- measure [& a]
    ...
    (object [:markers (cadr (memq :restrictions a)) :procedure (list (list '*ORDINAL* a))]))
...
'SUBJ4
        ...
        (cond
            ...
            *subj-vb-backup-type1* (do (set! *subj-vb-backup-type1* nil) (GO 'SUBJ11))
            ...
            (move-pt 'C 'U '(REL-NOT-FOUND)) (do (setr *c* :subject (getr *pt* :relhead)) (setr *c* :relhead (getr *pt* :relhead)) (remove-f *pt* 'REL-NOT-FOUND) (GO 'VB))
...
'NPASV
        ...
        (when (and (and (nq 'PREP) (parse 'PREPG) (smrelate *h*)) (not *nn*)) (GO 'RETSM))
        (when (and (and (nq 'TIMW) (parse 'ADV 'TIMW) (or (smtime) (GO 'FAIL))) (not *nn*)) (GO 'RETSM))
        (when (and (and (not (cq 'BE)) (parse 'ADJG 'ADV) (or (smrelate *h*) (GO 'FAIL))) (not *nn*)) (GO 'RETSM))
        (when (and (and (parse 'NG 'TIME) (or (smtime) (GO 'FAIL))) (not *nn*)) (GO 'RETSM))
        (when (and (and (nq 'PLACE) (parse 'ADV 'PLACE) (or (smplace) (GO 'FAIL))) (not *nn*)) (GO 'RETSM))
        (when (and (and (nq 'BINDER) (parse 'CLAUSE 'BOUND) (or (smbind) (GO 'FAIL))) (not *nn*)) (GO 'RETSM))
        (when (and (and (nextword? *n* 'TO) (parse 'CLAUSE 'TO 'ADJUNCT) (or (smtoadj) (GO 'FAIL))) (not *nn*)) (GO 'RETSM))
...
'THERQ
        ...
        (when (and (and (nq 'TIMW) (parse 'ADV 'TIMW)) (not *nn*)) (GO 'FAIL (m! "THEREQ")))
...
'THEF
        ...
        (when (and (and (nq 'ADV) (parse 'ADV 'TIMW)) (not *nn*)) (GO 'FAIL (m! "THEF")))
...
'IMPER
        ...
        (when (and (and (nq 'ADV) (parse 'ADJG 'ADV 'VBAD)) (not *nn*)) (GO 'IMPOP))
        (when (and (and (nq 'ADV) (parse 'ADV 'TIMW)) (not *nn*)) (GO 'IMPOP))
'IMPE
...
'AS
        ...
        (if (and (parse nil 'AS) (parse 'NUMD 'NUMDAS) *nn* (parse nil 'AS)) (if *nn* (GO 'NUMD2) (GO 'FAIL (m! "AS"))) (GO 'FAIL (m! "AS")))
'AT
...
'ND3
        ...
        (if (parse 'NUMD 'NUMDALONE) (if *nn* (GO 'NUMD2) (GO 'FAIL (m! "NUMD"))) (GO 'FAIL (m! "NUMD")))
'TIME
...
'SMNG
        ...
        (or (smng1) (GO 'FAIL))
        (when-not (not (isq *h* 'POSS)) (if *nn* (GO 'POSS) (GO 'RETSM)))
...
'TO
        (when *labeltrace* (passing 'TO))
        (fq! 'NAGR)
        (when (and (and (parse nil 'NOT) (fq! 'NEG)) (not *nn*)) (GO 'FAIL (m! "NOT")))
...
'IMPER
        (when *labeltrace* (passing 'IMPER))
        (when (and (and (parse 'VB 'DO 'NEG 'INF) (fq! 'NEG)) (not *nn*)) (GO 'FAIL (m! "DONT")))
...
'NAGR
        ...
        (when-not (or *tmp* (and (= *tense* '(PAST-PRESENT)) (set! *tense* '(PAST)))) (GO 'FAIL (m! "NAGR")))
'NAUX
...
'UP
        ...
        (cond (not (term? *prev*)) (GO 'RETSM)
            (= *prev* '\,) (if (nextword? *n* '\,) (do (fq! 'LIST) (GO 'UP)) (GO 'LIST))
            (memq *prev* ['AND 'OR 'NOR 'BUT])
                (do (when (= *both* (firstword *h*)) (fq! 'BOTH))
...
'LIST
        ...
        (when-not (and (= *prev* '\,) (== (count *h*) 2) (isq *h* 'NG) (not (or (isq *h* 'PRONG) (isq (cdr *h*) 'PRONG))) (or (nextword? *n* '\,) (nil? *n*))) (GO 'FAIL (m! "CONJOINß-HOPELESS-LIST")))
        (flushme)
        (fq! 'APPOSITIVE)
        (GO 'RETSM)
'LISTA
