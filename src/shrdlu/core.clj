    ...
    (binding [*thtt* thtt' *thnf* nil *thwh* nil *thlas* nil *thttl* nil *thfst* nil *thfstp* nil]
        ...
        (let [a (if (term? *thtt*)
                    (let [x (getprop *thtt* 'THEOREM)]
                        ...
                        (when-not x
                            (terpri) (pr *thtt*) (ert "CAN'T THASSERT, NO THEOREM - THADD"))
    ...
    (let [THT1 nil THT3 nil THSV nil THT2 nil THI1 nil]
        ...
        (cond (and (term? thi) (not (= thi '?)) (not (number? thi)))
            ...
            (or (= thi '?) (memq (car thi) '(THV THNV)))
                ...
                (if *thfst* (RETURN 'THVRB) (SETQ THI1 'THVRB))
            :else (RETURN 'THVRB))
            ...
            ((lambda [XPAIR YPAIR]
                ...
                (cond (thvar? thx) (thgal thx *tholist*)
                    ...
                    (= (car thx) 'THRESTRICT)
                        ...
                        (if (= (cadr thx) '?)
                            (let [_ (cons '? (cons 'THUNASSIGNED (concat (cddr thx) nil)))] (SETQ thx '(THNV ?)) _)
                ...
                (cond (thvar? thy) (thgal thy *thalist*)
                    (= (car thy) 'THRESTRICT)
                        (if (= (cadr thy) '?)
                            (let [_ (cons '? (cons 'THUNASSIGNED (concat (cddr thy) nil)))] (SETQ thy '(THNV ?)) _)
...
(§ defn- THMATCHLIST [thtb thwh]
    ...
    (let [thb1 nil THB2 nil THL nil *thnf* nil *thal* nil tha1 nil tha2 nil THRN nil thl1 nil thl2 nil THRVC nil]
        ...
        (SETQ thb1 thtb)
    THP1 (when-not thb1
            (RETURN (if thl2 (concat thl1 thl2) thl1)))
    ...
    THP2 (cond
            ...
            (> THL THRN) (do (SETQ thl1 tha1) (SETQ THL THRN)))
        ;; GO BACK FOR ANOTHER PASS.
        (GO THP1)))
...
(defn- thtry [x]
    ...
    (cond ;; ANY ATOMIC RECOMMENDATION IS IGNORED.  THIS IS USEFUL IN ERROR RECOVERY.
        ...
        (= (car x) 'THNUM)
            (list x)
        :else (do (terpri) (pr x) (thtry (ert "UNCLEAR RECOMMENDATION - THTRY")))))
...
(§ defn- THVAL [*thexp* *thalist*]
    ...
    GO  (SETQ THE *thexp*)
        ...
        (when-not (ERRSET (set! *thvalue* (eval THE)))
            ;; IF THERE WAS A LISP ERROR, REPORT IT TO THE USER.
            (terpri) (pr THE) (set! *thvalue* (ert "LISP ERROR - THVAL")))
    ...
    (let [a nil]
        ...
        (cond
            ...
            (and THY (= (car x) 'THNV))
                ...
                (do (thrplaca (cdr a) 'THUNASSIGNED) x)
...
(§ defq- THVSETQ [tha]
    (loop-when [a tha] a => *thvalue*
        (if (cdr a)
            ...
            (do (terpri) (pr tha) (ert "ODD NUMBER OF GOODIES - THVSETQ")))
        (recur (cddr a))))
...
(§ defn- ETAOIN []
    ...
    THRU (set! *sent* (SETQ WORD (set! *punct* (SETQ POSS nil))))
        ...
        (print "READY")
        (terpri)
    CHAR (cond (== (TYIPEEK) 24) (do (READCH) (ert nil) (GO THRU)) ;; "CTRL-X" BREAK LEFT OVER FROM CMU
            (== (TYIPEEK) 3) (bug "ETAOIN: ABOUT TO READ EOF"))
        (SETQ c (ascii (uppercase-ify-char (TYI))))
        (cond (= c \space) (GO WORD)                     ;; DELIMITER
            (= c ALTMODE)
                (do (SETQ c (ascii (uppercase-ify-char (TYI))))
                    (cond (= c ALTMODE) (do (ert nil) (GO THRU)) ;; ALTMODE-ALTMODE
                        (= c 'C) (do (print (ascii 12)) (GO DO))         ;; ALTMODE-C
                        (= c 'R) (do (terpri) (GO DO))         ;; ALTMODE-R
                        (and (= c 'S) *savesent*)            ;; ALTMODE-S CAUSES THE LAST SENTENCE TYPED IN TO
                            (do (set! *sent* (car *savesent*))          ;; RETURNED AS THE SENTENCE TO BE INTERPRETED
                        ...
                        (= c 'N)                           ;; ALTMODE-N COMPLEMENTS THE NEWWORD FLAG, WHICH
                            ...
                            (do (SETQ NEWWORD (not NEWWORD) ALTN (not ALTN))
                                (GO CHAR))
                        :else (GO THRU)))
            (= c (ascii 127))
                (do (cond WORD (do (print (car WORD)) (SETQ WORD (cdr WORD)))
                        *sent* (do (print (car *sent*)) (set! *sent* (cdr *sent*))))
                    (GO CHAR))
            (= c (ascii 13)) (GO WORD)
            (memq c PUNCL)
                (do (set! *punct* c)                           ;; DELIMITER
    ...
    WORD (cond (nil? WORD) (GO CHAR)
            ...
            :else
                (let [? (getprop WRD 'FEATURES)]
                    (cond ? ?                           ;; IF A WORD HAS FEATURES, IT'S PROPERTIES ARE ALL SET UP IN THE DICTIONARY
    ...
    CUT (cond
            (sta WORD '(T \' N)) (do (SETQ RD (cdddr WORD)) (SETQ WORD (cons '* WORD)) (GO TRY))
            (sta WORD '(S \')) (do (SETQ WORD (cddr WORD)) (SETQ POSS WRD) (GO WORD))
            (sta WORD '(\')) (do (SETQ WORD (cdr WORD)) (SETQ POSS WRD) (GO WORD))
    ...
    LY  (when (and (memq (car RD) VOWEL) (not (= (car RD) 'E)) (memq (cadr RD) CONSO))
            (SETQ RD (cons 'E RD)))
        (when (memq 'ADJ (getprop (SETQ ROOT (READLIST (reverse RD))) 'FEATURES))
    ...
    TRY (SETQ FEATURES (getprop (SETQ ROOT (READLIST (reverse RD))) 'FEATURES))
        (cond (or FEATURES (and (SETQ x (getprop ROOT 'IRREGULAR)) (SETQ FEATURES (mod (getprop (SETQ ROOT (car x)) 'FEATURES) (cdr x)))))
    ...
    PUNC (when *punct*
            (cond (and (= *punct* '?) (nil? *sent*)) (do (HELP) (GO THRU))
                (memq *punct* FINAL)
                    (RETURN (car (set! *savesent* (cons (reverse *sent*) *punct*)))) ;; RETURN POINT !!!!!!!!!!!!!
                :else (set! *sent* (cons *punct* *sent*))))
    ...
    NO  (when NEWWORD
            ...
            (when-not ALTN (SETQ NEWWORD nil))
            (GO PUNC))
        (terpri)
...
(§ defn- MOVE-PT [& a]
    ...
    EX  (when (or (nil? L2) (nil? (SETQ L2 (cdr L2))))
            (GO TEST))
        (SETQ xx (car L2))
        (GO LOOK)
...
(§ defn- PTFIND [x y z]
    ...
    DOWN (when-not (MOVE-PT 'DLC 'PV '(memq foo (firstword *pt*))) (RETURN nil))
    ON  (cond (not= x (firstword *pt*)) (GO DOWN)
            (or (= y true) (MOVE-PT 'DF '(= (wordafter *pt*) y)))
                (loop [] (cond (eval z) *pt* (and (= y true) (MOVE-PT 'DF)) (recur))))))
              ...
              (RESTRICTIONSß (((!ANIMATE)) ((!PHYSOB)) (SMOBL (!PLACE)))
               ...
               PROCEDUREß (!EVAL
                   (doall (map (lambda [%PLNRPHRASE]
                        (condp = (car %PLNRPHRASE)
                            '!ON (list '!PUTON '!2 (cadr %PLNRPHRASE) '*TIME)
                            '!IN (list '!PUTIN '!2 (cadr %PLNRPHRASE) '*TIME)
                            (ert "PUT DEFINITION")))
                        (relations? SMOBL)))))))))))
...
(putprop! 'SIT 'SEMANTICS '((VB
        ((ITRNSL (RELATION
              (RESTRICTIONSß (((!PHYSOB)) (SMOBL (!PLACE)))
               PROCEDUREß (!EVAL
                   (doall (map (lambda [%PLNRPHRASE]
                        (if (memq (car %PLNRPHRASE) '(!ON !IN)) (list '!SUPPORT (cadr %PLNRPHRASE) '!1 '*TIME)
                            (ert "SIT DEFINITION")))
                        (relations? SMOBL)))))))))))
...
(§ defn- !BLUEPRINT [x]
    ...
    (let [a (loop-when [a nil x (cddaar (INTERP x))] x => a
                (cond (not= (caar x) 'THGOAL) (ert "!BLUEPRINT THGOAL")
...
(§ defn- SMIT [PRONOUN]
    ...
    (let [CANDIDATES nil]
        (when-not discourse? (ert "SMIT: DISCOURSE SWITCH NOT ON"))
        (and *mvb*
            (isq *mvb* 'DO)
            (cq 'OBJ1)
...
(defn- smone []
    (loop [x (loop-when [a *h*] a
        ...
        (cond (and (MOVE-PT 'C 'U 'U '(NG)) (smone2 (list (car *pt*)) x)) *sm*
            ...
            :else (ert "SMONE: CAN'T FIND REFERENT FOR \"ONE\""))))
...
;;          (BUILDRSS WORD-BEING-INTERPRETED 'NEG 'NEG)))
    ...
    (isq x 'QUEST) true
    (isq x 'CLAUSE) true
    :else (ert "SMCL-MODIFIERS ATTEMPTED TO PROCESS AN UNEXPECTED TYPE OF CONSTITUENT")))
        ...
        (or
            ...
            (MAP #(let [a (semantics %)]
                    (when (cdr a) ;; TRUE IF NODE HAD MULTIPLE INTERPRETATIONS
                        (terpri)
                        (pr a)
                        (setr % 'SEMANTICS (ert "DOBACKREF: RETURN AN OSS FOR BACKNODE")))
...
(§ defn- PLNR-GOALIFY [plnrphrase]
    ...
    (if (getprop (car plnrphrase) 'NOGOAL) plnrphrase
...
(defn- plnr-notify [NEG? %PLNRPHRASE]
    ...
    (cond (not NEG?) %PLNRPHRASE (= (car %PLNRPHRASE) 'THNOT) (cadr %PLNRPHRASE) :else (list 'THNOT %PLNRPHRASE)))
...
(defn- plnr-recommendify [%PLNRPHRASE]
    ...
    (let-when [e (getprop (car %PLNRPHRASE) 'THMLIST)] e
...
(§ defn- EXPAND [exp event]
    ...
    (cond (rss? exp)
        ...
        (= (car exp) '*ORDINAL*) (if ORDINAL (global-err "I CAN'T HANDLE TWO ORDINALS OR SUPERLATIVES AT ONCE") (do (SETQ ORDINAL (cadr exp)) '*ORDINAL*))
        (= (car exp) '!SUBST) (do (ert "EXPAND - IS !SUBST BEING HANDLED BY SOMEONE ELSE?") exp)
        :else
        ...
        (cond
            (= TYPE 'ALL)
                (ANSBUILD
                    ...
                    (if (nil? ANS) '((say "YES")) (cons '(say "NO, NOT") (prepput (namelist PHRASE 'INDEF ANS))))
                    true)
            (= TYPE 'HOWMANY)
                (ANSBUILD
                    (+ PLAUS (plausibility? RSS))
                        ...
                        (cond
                            ...
                            (= (car NUM) '<)
                                (cons (cond
                                        ...
                                        :else '(say "NO,"))
                                    (prepput (namesugar LENGTH REL)))
                            :else (ert "ANSREL -- FUNNY NUMBER"))
                        true))
            :else (ert "ANSREL-- FUNNY TYPE"))))
...
(§ defn- ELIZA [node]
    ...
    (let [x nil num (count (wordafter node))]
        (APPLY #'concat
            (MAPLIST (lambda [word]
                (cond
                    (not (< num (count word))) nil                ;; THIS KLUDGE STOPS IT AT THE END OF THE NODE
                    (SETQ x (assq (car word) '((I YOU) (ME YOU) (AM ARE) (ARE AM))))
                        (cdr x)                                   ;; WE RETURN LIST OF THE THING REALLY WANTED, SO
                    (= (car word) 'YOU)                           ;; THE APPLY APPEND CAN GET RID OF THE EMPTY ONES.
                        (do (SETQ x (FINDMOTHER word node))            ;; UNFORTUNATELY, FOR "YOU" IT IS NECESSARY TO
...
(§ defn- LISTNAMES [PHRASE spec names]
    ...
    (let [COUNT nil EXAM nil x nil RES nil ANS nil comma? false]
        ...
        (when (nil? names) (RETURN '(say "NOTHING")))
    UP  (SETQ COUNT 1)
        (SETQ EXAM (car names))
    ...
    BACK (when (SETQ x (assq (car EXAM) names))
        ...
        (SETQ RES
            (doall (map (lambda [PHRASE2]
                    (if (propname (caadr PHRASE2)) (car PHRASE2)
                        ;; ANSNAME PARSES THE PHRASE AND PUTS THE ...
                        ;; ANSONE SUBSTITUTES "ONE" IF POSSIBLE
                        (do (ANSNAME PHRASE2) (ONECHECK (car PHRASE2)))))
                RES)))
        (SETQ ANS (car RES))
    OUTPUT (cond
            (nil? (SETQ RES (cdr RES))) (RETURN ANS)
            (cdr RES) (do (SETQ comma? true) (SETQ ANS (concat ANS '((print \,)) (car RES))))
            :else (SETQ ANS (concat ANS (and comma? '((print \,))) '((say "AND")) (car RES))))
        (GO OUTPUT)))
...
(§ defn- NAMEOBJ [ITEM spec]
    ...
    (let [TYPEß nil TYPELIST nil TYPE nil NAMEß nil COLORß nil COLORLIST nil SIZEß nil SIZELIST nil CUBE nil NAME nil]
        (let [x (assq ITEM '((ßSHRDLU I) (ßFRIEND YOU)))]
            (when x
    ...
    =>  (*CATCH 'ABORT-PARSER
            (loop []
                ...
                (when (and (ERRSET (set! *pt* (set! *c* (parse 'CLAUSE 'MAJOR 'TOPLEVEL)))) *c*)
                    ...
                    (set! *nb* *sent*)
                    (set! *h* (daughters *c*))
                    (when-not (ERRSET (answer *c*)) (print (or *global-message* "I DON'T UNDERSTAND."))))
                (recur)))
        (GO =>)))
