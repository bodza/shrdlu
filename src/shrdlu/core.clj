...
(defn- memq [x a] (cond (nil? a) nil (= x (car a)) a :else (recur x (cdr a))))
(defn- assq [x a] (cond (nil? a) nil (= x (caar a)) (car a) :else (recur x (cdr a))))
(def- delq remove)
...
(defn- thadd [b]
    ...
    (let [[t a b']
        ...
        (loop-when [a' nil a a i 1 f true p nil] a => (recur nil a' 1 nil true)
            ...
            ;; IF IT RETURNS THOK, THE ASSERTEE IS NOT IN YET.
            ;; OTHERWISE WE GO AROUND AGAIN, STILL LOOKING FOR A NON VARIABLE ITEM TO DO THE EQ CHECK.
            (let-when [o (thadd- (car a) t i n f p b b')] o => nil
                (if (= o 'thvrb)
                    (recur (concat a' (list (car a))) (cdr a) (inc i) f p)
                    (do (loop-when-recur [a (cdr a) i (inc i)] a [(cdr a) (inc i)] (thadd- (car a) t i n nil p b b'))
                        (loop-when-recur [a a'      i 1]       a [(cdr a) (inc i)] (thadd- (car a) t i n nil p b b'))
                        b'))))))
...
(defn- thamong [& a]
    ...
    (let [v (thgal (if (= (caar a) 'thev) (thval (cadar a) *vars*) (car a)) *vars*)]
        (if (= (cadr v) :thunassigned)
            (do (thpush! *tree* [:thamong v (thval (cadr a) *vars*)]) nil)
            ;; IF ($? X) ASSIGNED, THAMONG REDUCES TO A MEMBERSHIP TEST.
            (memq (cadr v) (thval (cadr a) *vars*)))))

(defn- thamong'f [v a]
    ;; (CAR THTREE) = (THAMONG OLDBINDINGCELL (NEW VALUES))
    (if a
        ;; REPLACE OLD VALUE WITH NEW VALUE, POP NEW VALUES.
        ;; STORE AWAY TREE AND THALIST FOR POSSIBLE BACKTRACKING.
        (let [v [(car v) (car a)]]
            (set! *tree1* (cons [:thamong v (cdr a)] *tree*))
            (set! *vars1* (set! *vars* (conj *vars* v)))
            true)
        ;; NO NEW VALUES LEFT.  RETURN X TO THUNASSIGNED.
        (let [v [(car v) :thunassigned]]
            (set! *vars* (conj *vars* v))
            nil)))

(putprop! :thamong :thfail thamong'f)
(putprop! :thamong :thsucceed nil)
...
(putprop! :thand :thfail thand'f)
(putprop! :thand :thsucceed thand't)
...
(putprop! :thcond :thsucceed thcond't)

(defn- thnot [e]
    (set! *expr* (list 'thcond [e '(thfail)] ['(thsucceed)])))
    ...
    (cond (term? a) nil
        (= (car a) :thuse)
            (map #(let-when [w (getprop % :theorem)] (and w (= (car w) t)) => (bug! 'thtae "BAD THEOREM" %)
                ...
                (cdr a))
        (= (car a) :thtbf)
            (do (when-not (contains? @m :l) (swap! m assoc :l (thmatch x1 t)))
...
(defn- thass- [a assert?]
    ...
    (let-when [x (car a) [? x] (if (term? x) [true x] (let [x (thvarsubst x nil)] [(thpure x) x]))] ? => (bug! 'thass- "IMPURE ASSERTION OR ERASURE" x)
        ...
        (let-when [a (cdr a) x (if assert? (thadd x) (thremove x))] x
            ...
            (let [t (if assert? :thante :therasing)]
                ...
                (thpush! *tree* [(if assert? :thassert :therase) x])
...
(defn- thassert [& a] (thass- a true))
(defn- therase [& a] (thass- a nil))

(defn- thassert'f [x] (thremove (if (term? x) x (car x))) nil)
(defn- thassert't [x] x)

(putprop! :thassert :thfail thassert'f)
(putprop! :thassert :thsucceed thassert't)

(defn- therase'f [x] (thadd (if (term? x) x (car x))) nil)
(defn- therase't [x] x)

(putprop! :therase :thfail therase'f)
(putprop! :therase :thsucceed therase't)

(defn- thgal [x vars]
    (or (find vars (cadr x)) (bug! 'thgal "THUNBOUND" x)))

(defn- thasval [x]
    (let [v (thgal x *vars*)] (and v (not= (cadr v) :thunassigned))))

(defn- thbind [a]
    ;; WHEN WE ENTER A NEW THEOREM OR THPROG, WE MUST BIND THE NEW VARIABLES.  A IS THE VARIABLE LIST.
    (set! *vars0* *vars*) ;; THOLIST IS THE OLD THALIST.
    (when a ;; IF A IS NIL, THERE IS NOTHING TO DO.
        ...
        (loop-when a a => (thpush! *tree* [:thrembind *vars0*])
            ;; OTHERWISE ADD TO THALIST THE NEW BINDING CELL.  THE FIRST ELEMENT IS THE NAME OF THE VARIABLE.
            ;; IF THE ENTRY IS AN ATOM, WE ARE JUST GIVEN THE VARIABLE AND ITS INITIAL ASSIGNMENT IS "THUNASSIGNED",
            ;; I.E. NO INITIAL ASSIGNMENT.  OTHERWISE WE ARE GIVEN BOTH THE VARIABLE AND ITS INITIAL ASSIGNMENT,
            ;; SO MAKE THE SECOND ELEMENT OF THE BINDING CELL A POINTER TO THE INITIAL ASSIGNMENT.
            (set! *vars* (conj *vars* (if (term? (car a)) [(car a) :thunassigned] [(caar a) (eval (cadar a))])))
            ;; REPEAT FOR THE NEXT VARIABLE IN THE LIST.
            (recur (cdr a)))))

(defn- thrembind'f [x] (set! *vars* x) nil)
(defn- thrembind't [x] (set! *vars* x) *value*)

(putprop! :thrembind :thfail thrembind'f)
(putprop! :thrembind :thsucceed thrembind't)

(defn- thbranch [a b]
    ;; THBRANCH IS CALLED BY THANDT OR THPROGT AND WE ARE SUCCEEDING BACKWARDS.
    (when' (cdr a) => b
        ;; WE NOW SETQ THBRANCH TO NIL.  IF THE NEXT LINE ALSO SUCCEEDS, THVAL WILL LOOK FOR
        ;; A NIL THBRANCH TO INDICATE THAT IT SHOULD SETQ IT AGAIN TO THE POINT OF SUCCESS.
        (let [t' *tree1*] (set! *tree1* nil)
            ;; CADDAR THTREE IS THE SECOND OF THE THREE ARGS ON THE THPROG MARK, THBRANCH AND THABRANCH ARE POINTERS TO THE
            ;; THTREE AND THALIST RESPECTIVELY AT THE POINT WHERE WE HAD JUST SUCCEEDED.  IN GENERAL, BY THE TIME WE GET BACK
...
(putprop! :thundo :thfail thundo'f)
(putprop! :thundo :thsucceed thundo't)
                          ...
                          [? t1] (cond
                            (and a2 (= (car x) 'thprog) (memq a2 (cadddr x)))
                                (loop-when [x (cddr x)] (car x) => [false (cdr t1)]
                                    (if (= (caaddaar x) a2)
                                        [true nil]
                                        (do (RPLAC (car x) (cdar x)) (recur x))))
                            (or (= (car x) 'thprog) (= (car x) 'thand))
                                (do (RPLAC (caddr x) nil) [(= (car x) (car a)) (cdr t1)])
                            (= (car x) :thrembind)
                                [(= (car x) (car a)) (cdr t1)]
                            :else
                                (do (RPLAC (cdr t1) (cddr t1)) [(= (car x) (car a)) t1]))
                    ] ? => (recur t1)
                        (set! *tree* (cdr *tree*))
                        true))))))
...
(putprop! :thfind :thsucceed thfind't)
    ...
    (when-not (term? x)
        (condp = (car x)
            ...
            :thtbf
                ...
                (do (when-not (contains? @m :t) (swap! m assoc :t (thmatch a1 :thconse)))
                    (when (:t @m) [[:thtbf (cadr x) (:t @m)]]))
            ;; DO THE SAME THING, ONLY FOR DATABASE FILTERS.
            :thdbf
                (do (when-not (contains? @m :d) (swap! m assoc :d (thmatch a1 :thassertion)))
            ...
            :thtbf (loop [b b]
                    (let-when [[_ b2 b3] (car b)] b3 => (recur (cdr b))
                        (let-when [t (car b3) w (getprop t :theorem)] (and w (= (car w) :thconse)) => (bug! 'thtry- "BAD THEOREM" t)
                            (let-when [? (and (b2 (car b3)) (thapply- w a)) b (cons [:thtbf b2 (cdr b3)] (cdr b))] ? => (recur b)
                                (thpush! *tree* [:thgoal a b n])
                                true)))))))
...
(defn- thmatch2 [x y a'undo]
    ...
    (let [x (if (= (car x) 'thev) (thval (cadr x) *vars0*) x)
          y (if (= (car y) 'thev) (thval (cadr y) *vars*) y)]
        (cond ;; IF EITHER IS A ?, ANYTHING WILL MATCH, SO OK.
            (= x '?) true
            (= y '?) true
            ;; IF EITHER IS A VARIABLE, THINGS GET MESSY.
            (or (memq (car x) ['thv 'thnv]) (memq (car y) ['thv 'thnv]))
                ;; X AND Y PAIR ARE THE RESPECTIVE BINDING CELLS.
                ;; IF THX OR THY IS NOT A VARIABLE (I.E. THE OTHER IS), THEN X OR Y PAIR WILL BE NIL.
                (let [xpair (when (thvar? x) (thgal x *vars0*)) ypair (when (thvar? y) (thgal y *vars*))]
                    (cond
                        (and xpair ;; THX IS A VARIABLE.  THIS SEES IF THX IS UNASSIGNED.
                                (or (= (car x) 'thnv) (and (= (car x) 'thv) (= (cadr xpair) :thunassigned))))
                            ;; THY IS ALSO A VARIABLE.  THIS MEANS WE MUST DO THE MYSTERIOUS VARIABLE LINKING.
                            (if ypair
                                (do (swap! a'undo thrplacas (cdr xpair) (cadr ypair))
                                    (swap! a'undo thrplacds ypair (cdr xpair))
                                    true)
                                ;; IF THY IS NOT A VARIABLE, JUST ASSIGN THX TO THY.
                                ;; THRPLACAS WILL HACK THML, THE FREE VARIABLE FROM THMATCH1.
                                (do (swap! a'undo thrplacas (cdr xpair) y)
                                    true))
                        (and ypair ;; THY IS A VARIABLE AND THX IS EITHER A CONSTANT OR A PREVIOUSLY ASSIGNED VARIABLE.
                                (or (= (car y) 'thnv) (and (= (car y) 'thv) (= (cadr ypair) :thunassigned))))
                            ;; IF THX IS A VARIABLE, LINK.  OTHERWISE JUST ASSIGN THY TO THX.
                            (do (swap! a'undo thrplacas (cdr ypair) (if xpair (cadr xpair) x))
                                true)
                        ;; THX IS AN ASSIGED VARIABLE, SO JUST MAKE SURE ITS ASSIGNMENT IS EQUAL TO THY.
                        (and xpair (= (cadr xpair) (if ypair (cadr ypair) y))) true
                        ;; THX IS A CONSTANT, THY IS A VARIABLE, AND THEY ARE EQUAL.
                        (and ypair (= (cadr ypair) x)) true
...
(defn- thmatch1 [x y]
    ...
    (let-when [x (if (= (car x) 'thev) (thval (cadr x) *vars0*) x)] (= (count x) (count y)) => false
        ...
        (let [a'undo (atom nil)]
            ...
            (if (every? true? (map #(thmatch2 %1 %2 a'undo) x y))
...
(defn- thmatch [b t]
    ;; THTB IS A PATTERN WHICH EVENTUALLY IS TO BE MATCHED.  THWH SAYS IF IT IS AN ASSERTION, CONSEQUENT THEOREM, ETC.
    ...
    ;; THNF = COUNTER WHICH SAYS WHICH PATTERN ITEM WE ARE WORKING ON.  THAL = LENGTH OF PATTERN.
    (let [n (count b)]
        ;; THL IS THE LENGTH OF THE SHORTEST BUCKET FOUND SO FAR.  INITIALLY IT IS SET TO A VERY LARGE NUMBER.
        ;; THB1 WILL BE THE REMAINDER OF THE PATTERN TO YET BE WORKED ON.  WHEN IT IS NIL, WE ARE DONE, SO RETURN THE BUCKET.
        ;; THL1 IS THE BUCKET UNDER THE ATOM.  THL2 IS THE BUCKET UNDER THE VARIABLE IN THAT POSITION.
        ...
        ;; OR A VARIABLE IN A GIVEN POSITION, AND STILL MATCH.
        (loop-when [l 34359738367 l1 nil l2 nil a b i 0] a => (if l2 (concat l1 l2) l1)
            ;; THB2 IS THE ITEM WE ARE WORKING ON IN THIS PASS.  IF IT'S NOT A NORMAL ATOM, SKIP IT AND GO TO NEXT PASS.
            (let-when [x (car a) a (cdr a) i (inc i)] (and (term? x) (not (number? x)) (not= x '?)) => (recur l l1 l2 a i)
                (let-when [a1 (getprop x t)
                      ...
                      a1 (cond (not a1) '(0 0)
                            :else (let [a1 (assq i (cdr a1))] (cond
                            (not a1) '(0 0)
                            :else (let [a1 (assq n (cdr a1))] (or a1 '(0 0))))))
                ] (not a1) => (recur l l1 l2 a i)
                    (let [m1 (cadr a1) a1 (cddr a1)]
                        ...
                        (if (= t :thassertion)
                            ...
                            (let [a2 (getprop 'thvrb t)
                                  a2 (cond (not a2) '(0 0)
                                        ...
                                        (not a2) '(0 0)
                                        :else (let [a2 (assq n (cdr a2))] (or a2 '(0 0))))))
                                  m2 (cadr a2) a2 (cddr a2)
                                  ...
                                  [l l1 l2] (if (< l (+ m1 m2)) [l l1 l2] [(+ m1 m2) a1 a2])]
                                (recur l l1 l2 a i)))))))))
...
(defn- thmung'f [a] (doseq [f a] (eval f)) nil)
(defn- thmung't [_] *value*)

(putprop! :thmung :thfail thmung'f)
(putprop! :thmung :thsucceed thmung't)

(defn- thprog- [a b]
    ;; ODD CASE WHERE THE THPROG HAS NO SUBEXPRESSIONS.  RETURN SUCCESS.
    (when' (cdr a) => 'thnoval
        ;; IF NEXT ITEM IS AN ATOM, IT'S A THPROG TAG.  USE THEXP TO MARK IT ON THTREE.
        ;; OTHERWISE NEXT EXPRESSION TO BE EVALUATED IS THE NEXT EXPRESSION OF THE THPROG.
        (let [x (cadr a)] (set! *expr* (if (term? x) (list 'thtag x) x)))
        ;; MOVE POINTER TO NEXT EXPRESSION.
        (thpush! *tree* [:thprog (cdr a) b])
        *value*))
...
(putprop! :thprog :thsucceed thprog't)
...
(defn- thremove [b]
    ...
    (let [[t a] (if (term? b) (let [a (getprop b :theorem)] [(car a) (caddr a)]) [:thassertion b]) n (count a)]
        (loop-when [a' nil a a i 1 f true p nil o nil] a => (recur nil a' 1 nil true o)
            (let-when [o (threm- (car a) t i n f p b o)] o => nil
                (if (= o 'thvrb)
                    (recur (concat a' (list (car a))) (cdr a) (inc i) f p o)
                    (do (loop-when-recur [a (cdr a) i (inc i)] a [(cdr a) (inc i)] (threm- (car a) t i n nil p b o))
                        (loop-when-recur [a a'      i 1]       a [(cdr a) (inc i)] (threm- (car a) t i n nil p b o))
                        o))))))
    ...
    (when a
        (let [a (if (= (car a) 'theorem) (cons 'thprog (cdr a)) a)]
            ...
            (loop-when [] *tree* => (bug! 'thsucceed "OVERPOP" a)
                (condp = (caar *tree*)
                    :thrembind
                        (do (set! *vars* (cadar *tree*)) (thpop! *tree*) (recur))
                    (car a)
                        (do (thpop! *tree*) (if (cdr a) (eval (cadr a)) 'thnoval))
                    (when' (and (= (car a) 'thtag) (= (caar *tree*) 'thprog)) => (do (thpop! *tree*) (recur))
                        (let-when [x (memq (cadr a) (cadddar *tree*))] x => (do (thpop! *tree*) (recur))
                            (RPLAC (cadar *tree*) (cons nil x))
                            (thprog't))))))))
...
(defn- thv- [x]
    ...
    (let [v (cadr (or (find *vars* x) (bug! 'thv- "THUNBOUND" x)))]
        (if (= v :thunassigned) (bug! 'thv- "THUNASSIGNED" x) v)))

(defn- thv [x] (thv- x))
(defn- thnv [x] (thv- x))

(defn- thval [expr' vars']
    ;; CORRESPONDS TO LISP EVAL.
    ;; THEXP IS THE EXPRESSION TO BE THVALUATED.  THALIST IS THE VARIABLE BINDING LIST.
    (binding [*expr* expr' *vars* vars' *vars0* nil *tree* nil *tree1* nil *vars1* nil *value* 'thnoval]
                            ...
                            (let-when [? (cond
                                        ...
                                        *value*
                                            (do ;; SAVE CURRENT STATE OF THTREE AND THALIST IN CASE WE HAVE TO BACK UP.
                                                (when-not *tree1* (set! *tree1* *tree*) (set! *vars1* *vars*))
                                ...
                                (let [f *expr* a (cdar *tree*)] (set! *expr* nil) (thpop! *tree*) (set! *value* (apply f a))
...
(defn- thvars2 [x y]
    ...
    (if (term? x) x
        ...
        (let [x (if (= (car x) 'thev) (thval (cadr x) *vars*) x)]
...
(defn- thvarsubst [x y]
    ...
    (let [x (cond (= (car x) 'thev) (thval (cadr x) *vars*) (thvar? x) (eval x) :else x)]
        ;; UNLESS THE ASSERTEE IS A THEOREM NAME, GO THROUGH IT PLACE BY PLACE WITH THVARS2.
        (if (term? x) x (doall (map #(thvars2 % y) x)))))

(defn- thsgal [x]
    (or (find *vars* (cadr x)) (let [v [(cadr x) :thunassigned]] (set! *vars* (conj *vars* v)) v)))
...
(defn- thconse [& a] (thdef :thconse a))
(defn- therasing [& a] (thdef :therasing a))

(defn- thdef [type a]
    ;; DEFINES AND OPTIONALLY ASSERTS (ANTECEDENT / CONSEQUENT / ERASING) THEOREMS
    (let [[name body] (if (term? (car a)) [(car a) (cdr a)] [(gensym (condp = type :thconse 'TC-G :thante 'TA-G :therasing 'TE-G)) a])
        ...
        (cond
            ...
            (thass- [name] true) (-print "DEFINED AND ASSERTED")
    ...
    [:thconse '[x y w z]
        ...
        '(thcond
            ...
            '[(thgoal '[!GRASPING ($? x)])
                ...
                (thgoal '[!MOVEHAND2 ($? y)] '[:thnodb] '[:thuse TC-MOVEHAND2])
                (thputprop ($? x) :history
                    (cons [*thtime* ($? z) (cadar (or (thval '(thgoal '[!SUPPORT ($? y) ($? x)]) (conj *vars* ['y :thunassigned])) [[nil 'ßHAND]])) nil]
                        (getprop ($? x) :history)))]
            '[(thgoal '[!MOVEHAND2 ($? y)] '[:thnodb] '[:thuse TC-MOVEHAND2])])])
...
(putprop! 'TCT-2 :theorem
    [:thconse '[x ev time]
        '[($? x) ($? time)]
        '(thgoal '[($? x) ($? ev) ($? time)] '[:thuse TCTE-3])])

(putprop! 'TCT-3 :theorem
    [:thconse '[x y ev time]
        '[($? x) ($? y) ($? time)]
        '(thgoal '[($? x) ($? ev) ($? y) ($? time)] '[:thuse TCTE-4])])
                        ...
                        (let [a (atab surf)]
                            (cond (= type 'CENTER)
                                    (let [v (list (max 0 (+ (caadr a) (half (- (caaddr a) (car size)))))
                                                  (max 0 (+ (cadadr a) (half (- (cadaddr a) (cadr size)))))
                                                  (+ (caddadr a) (caddaddr a)))]
                                        [nil (when (clear v size obj) v)])
                                (= (car a) 'ßBOX)
                                    [[(list (caadr a) (cadadr a)) (list (+ (caaddr a) (caadr a)) (+ (cadaddr a) (cadadr a))) 1] nil]
                                :else
                                    (let [x1 (half (car size)) y1 (half (cadr size))]
                                        [[(list (max 0 (- (caadr a) x1)) (max 0 (- (cadadr a) y1)))
                                          (list (min (dec (+ (caaddr a) (caadr a) x1)) 640) (min (dec (+ (cadaddr a) (cadadr a) y1)) 640))
                                          (+ (caddadr a) (caddaddr a))] nil]))))
            ] ? => _
                ...
                (not-every? nil? (map (lambda [a]
                        (when (and (not (memq (car a) obj)) (< (caadr a) max1) (< (cadadr a) max2))
                            (let-when [x (+ (caadr a) (caaddr a))] (< min1 x)
                                (let-when [y (+ (cadadr a) (cadaddr a))] (< min2 y)
                                    (when (< loc3 (+ (caddadr a) (caddaddr a)))
                                        (let [_ (cond
...
(defn- packo [obj type]
    (let [a (reduce (fn [a x] (if (thval (list 'thgoal ['!IS '($? x) type]) {'x x}) (packord x (size x) a) a)) nil obj)]
        (doall (map cadr a))))
...
(defn- packon [surf l]
    (let [surf (atab surf)]
        (loop-when l l
            (let [x (size (car l))]
                (if (or (< (caaddr surf) (car x)) (< (cadaddr surf) (cadr x)) (< 321 (+ (caddr x) (caddadr surf) (caddaddr surf))))
                    (recur (cdr l))
                    (car x))))))
...
(grammar! CLAUSE [*position-of-prt* nil *mvb* nil *locationmarker* nil *subj-vb-backup-type1* nil *position-of-ptw* nil]
    ...
    'NPASV
        ...
        (| (and (nq 'PLACE) (parse 'ADV 'PLACE) (or (smplace) (GO 'FAIL)))                 ;; PLACE
            ...
            nil
            'RETSM)
        (| (and (nq 'BINDER) (parse 'CLAUSE 'BOUND) (or (smbind *c*) (GO 'FAIL)))              ;; BINDER
            nil
            nil
            'RETSM)
    ...
    'POLAR
        (| (and (nq 'VB) (parse 'VB 'AUX '(QAUX)) (setr *c* :qaux *h*) (smvaux *c*) (setmvb *h*))
            nil
            'QCHOP)
...
(grammar! NG nil
    ...
    'PROPS
        (or (smprop *c*) (GO 'FAIL))                            ;; EXAMINE ITS SEMANTICS
    ...
    'SMOF
        (fq! 'OF)
        (| (or (smngof *c*) (not (pop*))) 'RETSM 'INCOM)
...
(grammar! VG [*tense* nil]
    ...
    'IMPER
        ...
        (| (and (parse 'VB '(MVB) 'INF) (setmvb *h*) (smvg *c*))
            'RETURN
...
(grammar! ADJG nil
    ...
    'RETSM
        ...
        (| (smadjg-prepg *c*) 'RETURN "SMADJ"))                 ;; FAIL IF ONE WAS NOT FOUND.
...
(grammar! CONJOIN [*prev* nil]
    ...
    'UP
        ...
        (| (cond (memq (car *rest*) ['ADJ 'NUM 'NOUN 'PREP 'VB 'ADV])
            ...
            (= (car *rest*) 'CLAUSE)
                (let [_ (set! *lastsent* (if (isq *h* 'MAJOR) *h* *lastsent*)) auxfe (meet (features *h*) ['DECLAR 'IMPER])]
    ...
    (if (getprop x :refer) '*2*
        (let [a (loop-when [a nil x (cddaar (§ INTERP x))] x => a
                    (cond (not= (caar x) 'thgoal) (bug! '!blueprint nil)
                        ...
                        (= (caadar x) '!PART) (recur (cons (cadadar x) a) (cdr x))
...
(defn- smconj- [x a l]
    ...
    (if l (doall (mapcat #(smconj- % (cons % a) (cdr l)) (semantics l)))
        (list (build
            :rssnode (when (rss? x) (gensym 'RSS))
            :ossnode (when (oss? x) (gensym 'OSS))
            :markers (markers? x)
            :systems (systems? x)
            :rel (rel? x)
...
(defn- smconj [a]
    ...
    (smset (smconj- nil nil a)))

(defn- smvg [node] ;; CALLED INSIDE ANY VG
...
(defn- smvaux [node]
    ...
    (putprop! (getr node :time) :tense (or (meet (features *h*) ['PRESENT 'PAST 'MODAL]) (bug! 'smvaux "FUNNY TENSE"))))
...
(defn- smadjg-prepg [node]
    ...
    (if (or (cq 'AGENT) (cq 'OF)) true
        (do (setr node :logical-subject
            ...
            (let [smsub (semantics (getr node :logical-subject))]
                (when (and (cq 'ADJG) (getr node :obj1))
                    (setr node :adjghead (compare-build (getr node :head) (cond (cq 'AS) '!ASMUCH (cq 'THAN) '!MORE :else (bug! 'smadjg-prepg "FUNNY TYPE")))))
                (if (getr node :obj1) (do (smcl1 node) *sm*)
                    (smset (binding [*sm* nil]
                        (smset (doall (map
                           #(build
                            ...
                            smsub)))
                        (eval (if (or (cq 'COMPAR) (cq 'SUP)) (findmeasure (getr node :head)) (semantics (getr node :head))))
                        *sm*)))))))

(defn- smit- [node pronoun a'candidates plausibility]
    ...
    (when (and node (getr node :head) (not (memq (car node) @a'candidates)) (if (= pronoun 'IT) (and (isq node 'NS) (not (isq node 'PRONG))) (isq node 'NPL)))
        (reset! a'candidates (cons (car node) @a'candidates))
        (smset (concat
            (doall (map (lambda [x]
                (let [name (gensym 'OSS)] (build
                    :ossnode name
...
(defn- smit [pronoun]
    ...
    (when' discourse? => (bug! 'smit "DISCOURSE SWITCH NOT ON")
        (let-when [a'candidates (atom nil)
                   last- (lambda []
                            (smit- (getr *lastsent* :subject) pronoun a'candidates 192)
                            (smit- (parsenode? *lastrel*) pronoun a'candidates 128)          ;; TRY REL (I.E. QUESTION FOCUS) OF THE LAST SENTENCE
                            (move-pt :lastsent :dlc)
                            (loop-when (move-pt [:pv 'NG])                      ;; GO THROUGH TOP LEVEL NG'S OF LAST SENTENCE
                                (smit- *pt* pronoun a'candidates 64)
                                (when (move-pt :pv) (§ recur)))
                            (when-not *sm*                                      ;; FIND A REFERENT MAP IN ANSNAME (NG'S IN LAST ANSWER)
                                (dorun (map* #(smit- % pronoun a'candidates 0) *ansname*)))
                            (when-not *sm*                                      ;; FIND A REFERENT MAP IN BACKREF2 (NG'S IN LAST SENTENCE)
                                (dorun (map* #(smit- % pronoun a'candidates 0) *backref2*))))
                   done- (lambda []
                            (putprop! pronoun :bind @a'candidates)
                            (when-not (cdr *sm*) (remprop! (car *sm*) :ambiguities)))
        ] (not (and *mvb* (isq *mvb* 'DO) (cq 'OBJ1))) => (smset lastevent)
            (cond (getprop pronoun :bind)
                    (dorun (map* #(smit- % pronoun a'candidates 0) (getprop pronoun :bind)))
                (smit- (getprop pronoun :lastbind) pronoun a'candidates 0)
                    (done-)
                (or (move-pt :c :u [:u 'NG] :u [:u 'NG]) (move-pt :c :u [:u 'NG] [:u 'COMP] [:pv 'SUBJ]))
                    (do (smit- *pt* pronoun a'candidates 0)
                        (move-pt :c :u [:u 'NG])
                        (when (isq *pt* 'DEF)
                            (add-f *pt* 'INDEF)
                            (remove-f *pt* 'DEF)
                            (doseq [x (semantics *pt*)] (putprop! x :determiner [['EXACTLY 1] 'INDEF nil]))))
                (or (move-pt :c [:u 'BOUND] :u) (move-pt :c [:u #(and (isq *pt* 'CLAUSE) (isq *pt* 'COMPONENT))] :u :dlc))
                    (do (smit- (getr *pt* :obj2) pronoun a'candidates 0)
                        (smit- (getr *pt* :obj1) pronoun a'candidates 0)
                        (smit- (getr *pt* :subject) pronoun a'candidates 0)
                        (when (and (nil? *sm*) (isq *pt* 'RSQ))
                            (smit- (getr *pt* :relhead) pronoun a'candidates 0))
...
(defn- smng2 [node]
    ...
    ;; DON'T USE FAKEY ANSWER NAME NODES FOR REFERENCE.  QUEST NODES ARE NOT SUITABLE REFERENTS.
    ;; SAVE THIS NG AWAY FOR POSSIBLE LATER BACK REFERENCE.
    ;; GO THRU ALL THE POSSIBLE INTERPRETATIONS OF THIS NOUN GROUP.
    (when (and (not (cq 'ANSNAME)) (getr node :head) discourse?)
        (set! *backref* (cons (car node) *backref*)))
...
(defn- smposs [node]
    ...
    (let [node (smposs2 node (move-pt :h [:pv 'POSS]))]
...
(defn- smrelate [node]
    ...
    (let [a (doall (map
               #(let-when [rel (rel? %)] (memq rel *sm*) => (bug! 'smrelate "TO WHOM?")
                    (build
                        :ossnode (when (oss? rel) (gensym 'OSS))
                        :rssnode (when (rss? rel) (gensym 'RSS))
                        :markers (or (when (relmarkers? %) (car (relmarkers? %))) (markers? rel))
                        :systems (or (when (relmarkers? %) (cadr (relmarkers? %))) (systems? rel))
                        :plausibility (plausibility? %)
...
(defn- smcl1 [node]
    (binding [*smsub* (getr node :logical-subject) *smob1* nil *smob2* nil *smobl* nil *smcomp* nil *rellist* nil]
        ...
        (set! *smsub*
            (cond *smsub* (semantics *smsub*)
                (cq 'IMPER) ['SHRDLU-OSS]
                (not (cq 'PASV)) (semantics (or (getr node :subject) (bug! 'smcl1 "NO SUBJECT")))
                (cq 'AGENT) (bug! 'smcl1 "AGENT MISSING")
                :else ['UNKNOWN-OSS-BY]))
        (set! *smob1* (semantics (getr node (if (cq 'PASV) :subject :obj1))))
        (set! *smob2* (semantics (getr node :obj2)))
        (set! *smobl* (semantics (getr node :lobj)))
        (set! *smcomp* (semantics (getr node :comp)))
        ...
        (set! *rellist*
            (semantics (cond (cq 'RSQ) (getr node :relhead) (or (cq 'PREPG) (cq 'ADJG)) (getr node :logical-subject) (cq 'QUEST) (getr node :relhead))))
        ...
        (let [sense-of-verb
                (cond
                    (cq 'PREPG) (semantics (set! *word-being* (getr node :head)))
                    (cq 'ADJG) (semantics (set! *word-being* (getr node :adjghead)))
                    :else (cadr (assq (car (meet *fe* ['ITRNS 'TRANS 'INT 'TRANSL 'TRANS2 'THERE 'ITRNSL])) (semantics (set! *word-being* (getr node :mvb))))))]
...
(defn- smcl-modifiers [a]
    ...
    (cond (nil? (getprop a :features)) true
...
;;          (BUILDRSS WORD-BEING-INTERPRETED 'NEG 'NEG)))
    ...
    (isq a 'NG)
        (and (cond (isq a 'TIM) true
                (and (cq 'REL-NOT-FOUND) (isq a 'QUEST) (isq (daughters a) 'TIM1))
                    (do (rq 'REL-NOT-FOUND)
                        (fq! 'TIMEQ)))
            (smtime))
    ...
    (isq a 'PREPG)
        (or (isq a 'AGENT)
            (bug! 'smcl-modifiers "BAD PREPG" a))
    (isq a 'QADJ)
    ...
    (isq a 'QUEST) true
    (isq a 'CLAUSE) true
    :else (bug! 'smcl-modifiers "ATTEMPTED TO PROCESS AN UNEXPECTED TYPE OF CONSTITUENT" a)))
...
(defn- istense [node arg]
    (let-when [x (getr node :time)] x => (bug! 'istense "NO TIME REGISTER")
        (let-when [x (tense? x)] x => (bug! 'istense "NO TENSE")
            (condp = arg
...
(defn- build
    ...
    (let [? (and rssnode (not markers))
          ...
          [markers systems] (if ? (check-markers markers nil nil) [markers systems]) m (if ? (assoc m :markers markers :systems systems) m)
                    ...
                    (let [restrictions
                        ...
                        (when (every? true?
                            ...
                            (let [name (gensym 'RSS)] (build
                                ...
                                :plausibility (+ (plausibility? *smsub*) (plausibility? *smob1*) (plausibility? *smob2*) (or plausibility 0))
            ...
            (let [{:keys [markers plausibility paraphrase procedure]} a]
                ...
                (let-when [ms* (check-markers markers (markers? oss) (systems? oss))] ms*
                    ...
                    (build
                        ...
                        :plausibility (+ (or plausibility 0) (plausibility? oss))))))
            '*sm* 'a)))
...
(defn- junkify- [code]
    ...
    (cond
        (term? code) (list code)
        (= (car code) 'thgoal) (list code '(putprop! *value* :who *sentno*))
        (= (car code) 'thfind) (list code (list 'thputprop (quotify (cadaddr code)) :bind '*value*))
        (any = (car code) 'thand 'thprog) (list (doall (mapcat junkify- code)))
...
(defn- plnr-junkify [code]
    ...
    (cond
        (term? code) code
        (= (car code) 'thgoal) (list 'thand code '(putprop! *value* :who *sentno*))
        (= (car code) 'thfind) (list 'thand code (list 'thputprop (quotify (cadaddr code)) :bind '*value*))
        (any = (car code) 'thand 'thprog) (doall (mapcat junkify- code))
...
(defn- plnr-mung [e candidates]
    ...
    (list* (car e) (cadr e) (caddr e) (cadddr e) (list 'thamong (caddr e) (quotify candidates)) (cddddr (if (= (caadddr e) 'thfind) (cdr e) e))))
...
(defn- expand [expr event freevars]
    ...
    (cond (rss? expr)
        ...
        :else
            (let [multiple (eval (getprop (car expr) :multiple)) a'quantifier (atom nil) a'choice (atom nil) a'var (atom nil) a'body (atom nil)
                  expr (doall (map (lambda [x]
                            (cond (or (not (term? x)) (not (or (rss? x) (oss? x)))) x
                                (refer? x)
                                    (if (cdr (refer? x)) (if multiple (do (erqset 'AND a'quantifier) (reset! a'choice (refer? x)) '*AND*) (refer? x)) (car (refer? x)))
                                (memq (variable? x) freevars)
                                    (do (when (rssvar? (variable? x)) (putprop! (variable? x) :used true)) (list 'thv (variable? x)))
                                (reset! a'choice (conjuncts? x))
                                    (do (erqset 'AND a'quantifier) (when (and multiple (refer? x)) (reset! a'choice (refer? x))) '*AND*)
                                (reset! a'choice (disjuncts? x))
                                    (do (erqset 'OR a'quantifier) '*OR*)
                                (cond (rss? x)
                                        (do (erqset 'EVENT a'quantifier) (putprop! (variable? x) :used true))
                                    (memq (quantifier? x) ['ALL 'NO])
                                        (do (erqset (quantifier? x) a'quantifier) true)
                                    (memq (quantifier? x) ['NDET 'INDEF])
                                        (do (cond (memq (num? x) ['NS 'SG-PL]) (erqset 'INDEF a'quantifier)
                                                (reset! a'choice (plnr-findspec (num? x))) (erqset 'FIND a'quantifier))
                                            true))
                                    (do (reset! a'body (plnr-describe (relations? x) (variable? x) (cons (variable? x) freevars)))
                                        (list 'thv (reset! a'var (variable? x))))
                                :else (bug! 'expand "STRANGE QUANTIFIER")))
                        (if event (list* (car expr) event (cdr expr)) expr)))]
                (condp = @a'quantifier ;; THE EVENT NAME IS STUCK INTO THE SECOND POSITION IF THERE IS ONE.
                    nil   (plnr-goalify expr)
                    'AND  (plnr-progify nil (doall (map #(expand (subst % '*AND* expr) nil freevars) @a'choice)))
                    'OR #_(PLNR-ORIFY (doall (map #(expand (subst % '*OR* expr) nil freevars) @a'choice))) (bug! 'plnr-orify "NOT WRITTEN YET")
                    'FIND (plnr-findify @a'choice @a'var (list @a'var) (plnr-progify nil (list @a'body (plnr-goalify expr))))
                    (plnr-notify (memq @a'quantifier ['ALL 'NO])
                        (plnr-progify (when @a'var (list @a'var)) (list @a'body (plnr-notify (= @a'quantifier 'ALL) (plnr-goalify expr)))))))))
...
(defn- check-a-marker [marker a'markers a'systems]
    ...
    (if (memq marker @a'markers) true
        (do (reset! a'markers (cons marker @a'markers))
            (loop-when [a (getprop marker :sys)] a => true
                (cond (memq (car a) @a'systems) nil                      ;; FAIL, IF SYSTEM IS THERE BY ANOTHER PATH
                    (check-a-marker (car a) a'markers a'systems) (do (reset! a'systems (cons (car a) @a'systems)) (recur (cdr a))))))))
...
(defn- check-markers [a markers systems]
    ...
    (let [a'markers (atom markers) a'systems (atom systems)]
        (loop-when a a => [@a'markers @a'systems]
            (recur-if (check-a-marker (car a) a'markers a'systems) (cdr a)))))
...
(defn- ansbuild [rss ans plausibility action rededuce?]
    ...
    (let [a'phrase (atom phrase)]
        ...
        (let-when [names (doall (map #(nameobj % spec) names))] names => '(say "NOTHING")
            (let [res (loop [res nil names names]
                ...
                (loop [res (doall (map #(if (propname (caadr %)) (car %) (do (ansname %) (onecheck (car %) a'phrase))) res)) ans (car res) comma? false]
...
(defn- nameaction [tense event]
    ...
    (let [a (caaddadadr (getprop event :thassertion)) verb (cutoff (car a)) obj1 (caddr a) obj2 (cadddr a)]
        (condp = verb
...
(putprop! 'TC-NAMEOBJ :theorem
    ...
    [:thconse '[[x *item*] type color name y z]
        '[!NAMEOBJ]
        '(thgoal '[!IS ($? x) ($? type)])
        '(set! *type* *value*)
        ...
        '(thcond
            ...
            '[(thgoal '[!IS ($? y) ($? type)])
                ...
                (set! *size* (namesize (size ($? x))))
                (= *size* (namesize (size ($? y))))
...
(defn- nameobj [item' spec]
    ...
    (binding [*item* item' *type* nil *typelist* nil *name* nil *color* nil *colorlist* nil *size* nil *sizelist* nil *cube* nil]
        (let-when [x (assq *item* [['ßSHRDLU 'I] ['ßFRIEND 'YOU]])] (not x) => (list (ansay (cdr x)) (list *item*))
            ...
            (disput *type*) ;; DISPUT CHECKS TO SEE IF DISCOURSE IS BEING KEPT, AND IF SO, PUTS THE RELEVANT SENTENCE NUMBER AS A PROPERTY ON THE ASSERTION.
            (let [type (caddar *type*)]
                (cond (= type '!NAME)
                    ...
                    :else
                        ...
                        (let-when [name (if *cube* '(CUBE) (list (cutoff type)))] (not *name*) => (list (ansay (list 'THE (car name) 'NAMED (caddar *name*))) (list *item*))
                            ...
                            (let-when [name (cons (cutoff (caddar *color*)) name)] (cdr *colorlist*) => (list (ansay (cons 'THE name)) (list *item*))
                            (let [name (cons *size* name)] (list
                                (cond
                                    (nil? (cdr *sizelist*))
...
(defn- findchoose [oss x ans']
    (cond (refer? oss) (atomify (refer? oss))
        (conjuncts? oss) (let [a'ans (atom ans')] (mapbland #(let [_ (findchoose % x @a'ans)] (reset! a'ans (concat _ @a'ans)) _) (conjuncts? oss)))
        (disjuncts? oss) (loop-when [a (disjuncts? oss)] a (or (findchoose (car a) x ans') (recur (cdr a))))
        :else
                    ...
                    (or (conjuncts? oss) (disjuncts? oss))
                        (let [ans (loop-when [ans nil a (or (conjuncts? (§ RSS)) (disjuncts? (§ RSS)))] a => ans
                                    (let [_ (getprop (car a) :refer)
                                          ans (if _ (concat _ ans) (let [_ (findchoose (car a) x (concat ans' ans))] (if _ (concat _ ans) ans)))]
                                    ...
                                    (cond ? (bind- ans)
                                        (not= x 'NOMORE)
                                            (let [? (plnr-findify (list 1 (- need have) true) (variable? oss) (list (variable? oss))
                                                        (plnr-progify nil
                                                            (concat (list code)
                                                                (subst (variable? oss) '*** '((not (or (memq (thv ***) ans) (memq (thv ***) ans')))))
                                                                (and x (subst (variable? oss) '* (car x))))))
                                                  ans (concat (thval ? *vars*) ans)]
...
(defn- showscene []
    ...
    (dorun (map (lambda [obj]
            ...
            (let [obj (thval '(thfind 'ALL '($? x) '[x] '(thgoal '[!SUPPORT ($? obj) ($? x)])) {'obj obj})]
                (when obj
                    (-print "SUPPORTS" obj))))
